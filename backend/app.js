...
// Cuánto tiempo se considera "todavía fresco" un rango ya sincronizado
// antes de volver a consultarlo en SAP. Se puede ajustar con la variable
// de entorno SYNC_CACHE_HORAS en Render sin tocar código.
const SYNC_CACHE_HORAS =
  parseFloat(
    process.env.SYNC_CACHE_HORAS
  ) || 2;

async function buscarSyncReciente(inicio, fin) {
  const result =
    await pool.query(
      `
      SELECT
    ...
    if (!forzar) {
      ...
      if (previo) {
        return res.status(200).json({
          ...
          mensaje:
            'Este rango ya se sincronizó hace poco (' +
            previo.registros +
            ' registros). Mostrando lo que ya está guardado sin volver a consultar SAP.',
